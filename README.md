...
│   ├── js/main.js         Nav, scroll-reveal, filters, video autoplay handling
│   ├── js/i18n.js         EN/NL dictionary + language switch logic
│   └── video/
...
`assets/video/hero-loop.mp4` is a generated placeholder (a slow, moody lime-on-black gradient loop) so the site works out of the box. Swap it for your own footage:

1. Export your video as **muted MP4 (H.264)**, ideally under ~15MB and 15–20 seconds, looping cleanly.
2. Replace `assets/video/hero-loop.mp4` with your file (keep the same filename, or update the `<source>` path in `index.html`).
3. Generate a poster frame (first frame as a still image) and replace `assets/video/poster.jpg` — this is what shows while the video loads, and on very low-power mobile connections.
...
- Either keep the generated gradient backdrop (edit the `--card-a / --card-b / --card-c` CSS variables in the inline `style=""` for a quick colour change) or replace `<div class="backdrop"></div>` with a real image, e.g.:
  ```html
  <div class="backdrop"><img src="assets/img/project-01.jpg" alt="Project name" style="width:100%;height:100%;object-fit:cover;"></div>
  ```
...
- `assets/js/i18n.js` holds one dictionary — `VINCIVE_I18N` — mapping each key to `{ en: "...", nl: "..." }`. On click, the matching language's text is swapped in via `innerHTML`, so tags like `<br>` or `<em>` inside a translation still work.
...
1. Find or add a key in `VINCIVE_I18N` inside `assets/js/i18n.js`, with both an `en` and `nl` value.
...
- **Colour system:** near-black base (`#0a0a0b`) with a single acid-lime accent (`#c6f135`) — defined as CSS custom properties at the top of `assets/css/style.css`, so the whole palette can be changed in one place.
